...
def main(
    ...
    for runner_concurrency, intraop_threads, interop_threads in itertools.product(
        runner_concurrency_list, intraop_threads_list, interop_threads_list
    ):
        logger.info(
            f"Model: {model_path}, Concurrency: {runner_concurrency}, IntraOp: {intraop_threads}, InterOp: {interop_threads}"
        )
